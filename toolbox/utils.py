...
def save_ckpt(model, prefix=''):
    state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
    torch.save(state,  prefix+'model.pth')
    #torch.save(model.state_dict(),  prefix+'model.pth')


# def save_ckpt(base_dir, model, prefix=''):
#     """
#     保存模型权重到指定路径
#     Args:
#         base_dir (str): 保存目录
...
#
#     state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
#     save_path = os.path.join(base_dir, prefix + 'model.pth')
#     torch.save(state, save_path)
#     print(f"Model saved at {save_path}")

def load_ckpt( model, prefix=''):
    save_pth = prefix
